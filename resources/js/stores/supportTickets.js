...
export const useSupportTicketsStore = defineStore('supportTickets', {
    ...
    actions: {
        async fetchList(params = {}) {
            const { data } = await api.get('/support-tickets', { params: cleanParams(params) })
        ...
        async raise(subject, message) {
            const { data } = await api.post('/support-tickets', { subject, message })
            return data.ticket
        },

        async addComment(ticketId, body) {
            const { data } = await api.post(`/support-tickets/${ticketId}/comments`, { body })
