...
<template>
    <div class="p-8">
        ...
        <h1 class="text-body mt-1 text-2xl font-bold">Student Submissions</h1>
        ...
        <div v-else-if="submissions.length === 0" class="mt-16 flex flex-col items-center text-center">
            <p class="text-muted">No students have submitted work for this activity yet.</p>
        </div>
        ...
        <div v-else class="mt-8 space-y-3">
            <button
                ...
                :key="submission.id"
                type="button"
                class="flex w-full items-center gap-4 rounded-2xl bg-card p-5 text-left shadow-elevated transition hover:brightness-95"
                @click="openSubmission(submission)"
            >
                <div class="min-w-0 flex-1">
                    <p class="text-body font-bold">{{ submission.student.first_name }} {{ submission.student.last_name }}</p>
                    <p class="mt-1 text-sm text-muted">
                        Attempt {{ submission.attempt_number }} &middot; Submitted {{ new Date(submission.submitted_at).toLocaleString() }}
                    </p>
                </div>
                <p v-if="submission.status === 'graded'" class="text-sm font-semibold text-body">
