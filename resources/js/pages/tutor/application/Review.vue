...
<template>
    ...
    <div v-else class="w-full max-w-lg">
        <div class="text-center">
            <h1 class="text-body text-3xl font-bold">Review your application</h1>
            <p class="mt-2 text-muted">Make sure everything looks right before you submit.</p>
        </div>
        ...
        <div class="mt-8 space-y-6">
            <section class="rounded-xl border border-border p-5">
                ...
                <dl class="mt-3 space-y-1 text-sm text-muted">
                    ...
                    <p><span class="font-medium text-body">Display name:</span> {{ app.display_name }}</p>
                    <p><span class="font-medium text-body">Bio:</span> {{ app.bio }}</p>
                    <p><span class="font-medium text-body">Experience:</span> {{ app.years_experience }} years</p>
                    <p><span class="font-medium text-body">Occupation:</span> {{ app.occupation }}</p>
                    <p><span class="font-medium text-body">Languages:</span> {{ app.languages.join(', ') }}</p>
                </dl>
            </section>
            ...
            <section class="rounded-xl border border-border p-5">
                ...
                <dl class="mt-3 space-y-1 text-sm text-muted">
                    <p><span class="font-medium text-body">Teaching style:</span> {{ app.teaching_style }}</p>
                    <p><span class="font-medium text-body">About me:</span> {{ app.about_me }}</p>
                    <p><span class="font-medium text-body">Why choose me:</span> {{ app.why_choose_me }}</p>
                </dl>
            </section>
            ...
            <section class="rounded-xl border border-border p-5">
                ...
                <ul class="mt-3 space-y-3">
                    <li v-for="qualification in app.qualifications" :key="qualification.id" class="text-sm text-muted">
                        <p class="font-medium text-body">{{ qualification.title }}</p>
            ...
            <section class="rounded-xl border border-border p-5">
                ...
                <p class="mt-3 text-sm text-muted">{{ app.government_id?.name ?? 'Not uploaded' }}</p>
            </section>

            <section class="rounded-xl border border-border p-5">
                <div class="flex items-center justify-between">
                    <h2 class="text-body font-bold">Supporting Documents</h2>
                ...
                <ul class="mt-3 space-y-1 text-sm text-muted">
                    ...
                    <li v-if="app.documents.length === 0" class="italic text-muted">None uploaded</li>
                </ul>
            </section>
        </div>
