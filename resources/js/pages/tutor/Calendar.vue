...
<template>
    <div class="p-8">
        <h1 class="text-ink text-2xl font-bold">Teaching Calendar</h1>
        <p class="mt-1 text-gray-500">Manage the days and times you're available to teach.</p>
        ...
        <div v-else class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm lg:col-span-1">
                <div v-if="!selectedDate" class="flex flex-col items-center py-12 text-center">
                    ...
                    <p class="mt-4 text-gray-500">Select a date to view or add availability.</p>
                </div>

                <template v-else>
                    <div class="flex items-center justify-between gap-3">
                        <h2 class="text-ink font-bold">{{ selectedDateLabel }}</h2>
                        <button
                            v-if="daySlots.length > 0"
                            type="button"
                            class="bg-amber flex shrink-0 items-center gap-1 rounded-full px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:brightness-95"
                            @click="openAddModal"
                        >
                    ...
                    <div v-if="daySlots.length === 0" class="mt-8 flex flex-col items-center text-center">
                        <p class="text-gray-500">No availability has been added for this date.</p>
                        <button
                            type="button"
                            class="bg-amber mt-5 flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:brightness-95"
                            @click="openAddModal"
                        >
                            <PlusIcon class="h-4 w-4" />
                            Add Time Slot
                    ...
                    <ul v-else class="mt-5 space-y-3">
                        ...
                        >
                            <span class="text-ink font-semibold">{{ slot.start_time }} - {{ slot.end_time }}</span>
        ...
        <Modal v-model="showFormModal" :title="isEditing ? 'Edit Time Slot' : 'Add Time Slot'">
            <form class="space-y-6" novalidate @submit.prevent="saveSlot">
                ...
                <div>
                    <p class="mb-1 text-sm font-medium text-gray-500">Selected Date</p>
                    <p class="text-ink font-semibold">{{ selectedDateLabel }}</p>
                </div>
        ...
        >
            <p class="text-gray-600">This time slot will no longer be available.</p>
            <template #footer>
                <button
                    type="button"
                    class="rounded-full border border-gray-300 px-5 py-2.5 font-semibold text-gray-700"
