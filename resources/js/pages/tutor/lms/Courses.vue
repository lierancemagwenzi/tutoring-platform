...
<template>
    <div class="p-8">
        ...
        <div v-else class="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <div v-for="course in store.courses" :key="course.id" class="flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm">
                ...
                <div class="flex flex-1 flex-col p-5">
                    <div class="flex items-start justify-between gap-2">
                        <p class="text-sm text-gray-500">{{ course.subject.name }} &middot; {{ course.grade.name }}</p>
                        <span class="shrink-0 rounded-full px-3 py-1 text-xs font-semibold" :class="statusClasses(course.status)">
                            {{ statusLabel(course.status) }}
                        </span>
                    </div>

                    <p class="text-ink mt-1 font-bold">{{ course.title }}</p>
                    <p class="mt-2 line-clamp-2 text-sm text-gray-500">{{ course.description }}</p>
