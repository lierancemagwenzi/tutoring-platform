...
const surveyContentId = computed(() => Number(route.params.id))

const loading = ref(true)
const saving = ref(false)
const actionError = ref('')
const surveyContent = ref(null)
...
onMounted(async () => {
    surveyContent.value = await store.fetchSurveyContent(surveyContentId.value)
    loading.value = false
})
...
async function save() {
    ...
    try {
        if (editingQuestion.value) {
            ...
            const index = surveyContent.value.questions.findIndex((question) => question.id === updated.id)
            if (index !== -1) {
                surveyContent.value.questions[index] = updated
            }
        } else {
            const created = await store.createQuestion(surveyContentId.value, buildPayload())
            surveyContent.value.questions.push(created)
        }
...
async function remove(question) {
    ...
    try {
        ...
        surveyContent.value.questions = surveyContent.value.questions.filter((entry) => entry.id !== question.id)
    } catch (error) {
...
async function onReorder() {
    try {
        await store.reorderQuestions(
            surveyContentId.value,
            surveyContent.value.questions.map((question) => question.id),
        )
    } catch (error) {
...
function openPreview() {
    previewModel.value = new Model({
        elements: surveyContent.value.questions.map((question) => ({
            type: question.type,
    ...
    <div class="p-8">
        ...
        <template v-else>
            <div class="flex items-center justify-between">
                <div>
                    <router-link to="/tutor/self-paced-survey-contents" class="text-accent text-sm font-semibold">
                        &larr; Back to Survey Banks
                    </router-link>
                    <h1 class="text-body mt-1 text-2xl font-bold">{{ surveyContent.title }}</h1>
            ...
            <div v-if="surveyContent.questions.length === 0" class="mt-16 flex flex-col items-center text-center">
                <p class="text-muted">No questions yet. Add your first question to get started.</p>
            </div>

            <draggable
                v-else
                v-model="surveyContent.questions"
                item-key="id"
                handle=".drag-handle"
        ...
        <Modal v-model="modalOpen" :title="editingQuestion ? 'Edit Question' : 'Add Question'">
            <form class="space-y-4" novalidate @submit.prevent="save">
                <p v-if="actionError" class="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-600">{{ actionError }}</p>
        ...
        <Modal v-model="previewOpen" title="Survey Preview">
