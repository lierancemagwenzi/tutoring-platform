...
<template>
    <div class="p-8">
        ...
        <p class="mt-1 text-sm text-gray-500">Answers to common questions.</p>
        ...
        <div v-else-if="store.faqs.length === 0" class="mt-16 text-center text-gray-500">No FAQs available yet.</div>

        <div v-else class="mt-6 space-y-3">
            <div v-for="faq in store.faqs" :key="faq.id" class="rounded-2xl bg-white shadow-sm">
                <button
                    type="button"
                    class="flex w-full items-center justify-between gap-4 px-5 py-4 text-left"
                    @click="toggle(faq)"
                >
                    <span class="text-ink font-semibold">{{ faq.question }}</span>
                    <ChevronDownIcon
                        class="h-5 w-5 shrink-0 text-gray-400 transition-transform"
                        :class="{ 'rotate-180': openId === faq.id }"
                    />
                </button>
                <p v-if="openId === faq.id" class="border-t border-gray-100 px-5 py-4 text-sm text-gray-600">
                    {{ faq.answer }}
                </p>
            </div>
        </div>
