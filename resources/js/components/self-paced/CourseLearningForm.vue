...
<template>
    <form class="max-w-2xl space-y-6 rounded-2xl bg-white p-6 shadow-sm" novalidate @submit.prevent="save">
        ...
        <div v-for="[field, label] in [['learning_objectives', 'Learning Objectives'], ['prerequisites', 'Prerequisites'], ['target_audience', 'Target Audience']]" :key="field">
            ...
            <div v-if="form[field].length === 0" class="mt-2 text-sm text-gray-400 italic">None added yet.</div>
            <div v-for="(item, index) in form[field]" :key="index" class="mt-2 flex items-center gap-2">
                <input
                    v-model="form[field][index]"
                    type="text"
                    class="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm text-gray-900 outline-none"
                />
                <button type="button" class="text-gray-400 hover:text-red-600" @click="removeItem(field, index)">
